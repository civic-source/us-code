---
title: "Section 5527 - Miscellaneous provisions"
usc_title: 15
usc_section: "5527"
chapter: 81
current_through: "PL 113-21"
classification: "15 U.S.C. § 5527"
generated_at: "2026-03-30T03:04:31.232Z"
status: "active"
---

# § 5527. Miscellaneous provisions

- **(a)** Except to the extent the appropriate Federal agency or department head determines, the provisions of this chapter shall not apply to—
  - **(1)** programs or activities regarding computer systems that process classified information; or
  - **(2)** computer systems the function, operation, or use of which are those delineated in paragraphs (1) through (5) of section 2315(a) of title 10 . 1 1 So in original. Section 2315 of title 10 does not contain a subsec. (a).
- **(b)** In accordance with Federal contracting law, Federal agencies and departments participating in the Program may acquire prototype or early production models of new high-performance computing systems and subsystems to stimulate hardware and software development. Items of computing equipment acquired under this subsection shall be considered research computers for purposes of applicable acquisition regulations.
